...
document
  ...
  .addEventListener("click", function () {
    ...
    function checkDividor(num) {
      for (var i = num; i >= 1; i--) if (num % i === 0) mess += i + ", ";
      return mess;
    }
  });
...
let bugsRate = (quality, workingDay) => {
  return ((5.5 - quality) * workingDay) / 10;
};
let workingDay = (estimation, productivity) => {
  return estimation / productivity;
};
let cost = (dailyRate, workingDay) => {
  return workingDay * dailyRate;
};
let endDate = (leadtime, workingDay) => {
  return workingDay + leadtime;
};
...
const calculator = (estimate, leadtime) => {
  for (const employee in resources) {
    const workingDate = workingDay(10, resources[employee].productivity);
    //   console.log(resources[employee]);
    //   console.log(workingDay(10, resources[employee].productivity));
    console.log(
      "Employee: ",
      employee,
      " bugsrate",
      bugsRate(resources[employee].quality, workingDate)
    );
    console.log(
      "Employee: ",
      employee,
      " cost",
      cost(resources[employee].dailyRate, workingDate)
    );
    console.log(
      "Employee: ",
      employee,
      " endate",
      endDate(leadtime, workingDate)
    );
  }
};
